...
// Platform:    Lenovo Z510,Core i7, Windows 8.1,Visual Studio 2013//
...
namespace XmlProcessing
{
  ...
  public:
	...
	virtual int getSelfClosing() = 0;
	virtual void SelfCloseTag(int setclosetag) = 0;
	virtual int getchildren() = 0;
	virtual void setchildren(int setchild)=0;
	virtual std::vector<std::shared_ptr<AbstractXmlElement>> Children() = 0;
	virtual std::vector<std::pair<std::string, std::string>> Attribute() = 0;
  protected:
    static size_t count;
  ...
  public:
	...
	virtual int getSelfClosing(){ return 0; }
	virtual void SelfCloseTag(int setclosetag){ }
	virtual int getchildren(){ return 0; }
	virtual void setchildren(int setchild){ }
	virtual std::vector<std::shared_ptr<AbstractXmlElement>> Children(){ return children_; }
	virtual std::vector<std::pair<std::string, std::string>> Attribute(){ std::vector < std::pair<std::string, std::string>> temp;  return temp; }
  private:
	   bool hasXmlRoot();
  ...
  std::shared_ptr<AbstractXmlElement> makeDocElement();
  ...
  public:
	...
	virtual int getchildren(){ return 0; }
	virtual void setchildren(int setchild){ }
	virtual std::vector<std::shared_ptr<AbstractXmlElement>> Children(){ std::vector<std::shared_ptr<AbstractXmlElement>> temp1;  return temp1; }
	virtual std::vector<std::pair<std::string, std::string>> Attribute(){ std::vector < std::pair<std::string, std::string>> temp;  return temp; }
  ...
  public:
	...
	TaggedElement(const TaggedElement&& movetagelement)
	{
		tag_ = movetagelement.tag_;
		attribs_ = movetagelement.attribs_;
		movetagelement.tag_.empty();
		movetagelement.attribs_.empty();
	}
	TaggedElement& operator=(const TaggedElement&& movetagelement)
	{
		if (this != &movetagelement)
		{
			tag_ = movetagelement.tag_;
			attribs_ = movetagelement.attribs_;
			movetagelement.tag_.empty();
    ...
    virtual std::string value();
    virtual std::string toString();
	virtual int getSelfClosing(){ return selfClosing; }
	virtual void SelfCloseTag(int setclosetag){ selfClosing = setclosetag; }
	virtual int getchildren(){ return numberofchildren; }
	virtual void setchildren(int setchild){ numberofchildren = setchild; }
	virtual std::vector<std::shared_ptr<AbstractXmlElement>> Children(){ return children_; }
  ...
  private:
	  std::string commentText_ = "comment_text";
  };
  ...
  private:
    ...
    std::vector<std::pair<std::string, std::string>> attribs_;
    std::string type_ = "procinstrelement";
    std::string text;
  };
  ...
  public:// changed here
	...
	XmlDeclarElement(){}
	XmlDeclarElement& operator=(const XmlDeclarElement& de) = delete;
    virtual bool addAttrib(const std::string& name, const std::string& value);
    virtual bool removeAttrib(const std::string& name);
